...
public class ItemController : MonoBehaviour
{
    ...
    private void ApplyCardEffect(string cardName)
    {
        switch (cardName)
        {
            ...
            case "card9":
                Debug.Log("스킬쿨타임감소!");
                DataManager.Instance.additionalSkillCoolDown += 0.5f;
                break;
            case "card10":
                Debug.Log("랜덤!");
        ...
        else if (rand <= 5 && rand > 2)
        {
            ...
            else if (rand == 5)
            {
                ...
                if (DataManager.Instance.Weapon == WeaponType.Sword.ToString())
                {
                    ...
                    card.transform.Find("Explain").GetComponent<TextMeshProUGUI>().text = "어디까지\n길어지는 거에요?";
                }
                else
                {
                    card.transform.Find("Effect").GetComponent<TextMeshProUGUI>().text = "도탄 횟수 증가\n+1";
                    card.transform.Find("Explain").GetComponent<TextMeshProUGUI>().text = "이제\n총알이 튕깁니다";
                }
            }
        }
        else
        {
            ...
            switch (rand % 6)
            {
                ...
                case 1:
                    ...
                    card.transform.Find("Explain").GetComponent<TextMeshProUGUI>().text = "하지만 빨랐죠?";
                    break;
                case 2:
                    card.gameObject.name = "card7";
                    card.transform.Find("Effect").GetComponent<TextMeshProUGUI>().text = "공격력 증가\n+0.5";
                    card.transform.Find("Explain").GetComponent<TextMeshProUGUI>().text = "힘이 곧 정의!";
                    break;
                case 3:
                    card.gameObject.name = "card8";
                    card.transform.Find("Effect").GetComponent<TextMeshProUGUI>().text = "체력 회복\n+1";
                    card.transform.Find("Explain").GetComponent<TextMeshProUGUI>().text = "죽을것 같다면야...";
                    break;
                case 4:
